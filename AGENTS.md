...
AI agents working in this repository must default to read-only behavior.

Agents may run read-only inspection commands such as `pwd`, `ls`, `rg`, `find`, `git status`, `git diff`, `git log`, `git show`, and file reads.

Agents must not run mutating commands unless the user explicitly asks for that exact action in the current conversation. This includes, but is not limited to:
...
When an action is needed, agents should provide the exact command for the user to run manually unless the user has explicitly authorized the agent to run that specific mutating command.
